...
if os.environ.get('COMPUTERNAME')=='CAPTAIN2020':
    with open(r"C:\Users\captian2020\Documents\config_files\config_web02.json") as config_file:
        config = json.load(config_file)
elif os.environ.get('COMPUTERNAME')=='NICKSURFACEPRO4':
    with open(r"C:\Users\Costa Rica\OneDrive\Documents\professional\config_files\config_web02.json") as config_file:
        config = json.load(config_file)
else:
    with open('/home/ubuntu/environments/config_web02.json') as config_file:
        config = json.load(config_file)
...
class ConfigProd:
    ...
    ADMIN_EMAILS = config.get('ADMIN_EMAILS')
    SQLALCHEMY_BINDS ={'dbCage':config.get('CAGE_DB'),
        'dbBls' : config.get('SQLALCHEMY_BINDS_BLS')}
    REGISTRATION_KEY =config.get('REGISTRATION_KEY')
    BLS_API_URL = config.get('BLS_API_URL')
