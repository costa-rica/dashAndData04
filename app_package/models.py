...
class Users(db.Model, UserMixin):
    ...
    posts_to_html = db.relationship('PostsToHtml', backref='authorToHTML', lazy=True)#ONE
...
class Posts(db.Model):
    ...
    posts_to_html = db.relationship('PostsToHtml', backref='postDetails', lazy=True)#ONE
    posts_to_html_chars = db.relationship('PostsToHtmlTagChars', backref='postDetailsChars', lazy=True)#ONE
...
class PostsToHtml(db.Model):
    ...
    posts_to_html_tag = db.relationship('PostsToHtmlTagChars', backref='postChars', lazy=True)#table with ONE
    
    def __repr__(self):
        return f"PostsToHtml(id: {self.id}, word_row_id: {self.word_row_id} , " \
            f"row_tag: {self.row_tag}, row_going_into_html: {self.row_going_into_html})"

class PostsToHtmlTagChars(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    # posts_to_html_id = db.Column(db.Integer)
    post_tag_characters = db.Column(db.Text)
    word_row_id = db.Column(db.Integer, db.ForeignKey(PostsToHtml.id), nullable=False)#table with MANY
    post_id = db.Column(db.Integer, db.ForeignKey('posts.id'), nullable=False)#table with MANY
    # posts_to_html_id = db.Column(db.Integer, db.ForeignKey('poststohtml.id'),nullable=False)#table with many
    

    def __repr__(self):
        return f"PostsBackup(id: {self.id}, " \
